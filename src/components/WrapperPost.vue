...
const backTo = computed(() => {
  const routePath = route.path.split('/').slice(0, -1).join('/')
  return routePath === '/categorys' ? '/posts' : routePath || '/'
})
...
<template>
  ...
  <div v-if="route.path !== '/'" class="prose m-auto mt-8 mb-8 slide-enter animate-delay-500 print:hidden">
    ...
    <RouterLink :to="backTo" class="font-mono op50 hover:op75" v-text="'cd ..'" />
  </div>
</template>
